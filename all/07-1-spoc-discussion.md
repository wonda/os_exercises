...
2. （spoc)了解race condition. 进入[race-condition代码目录](https://github.com/chyyuu/ucore_lab/tree/master/related_info/lab7/race-condition)。
 ...
 > 答：dx没有赋值，所以初始值为0，循环一次之后dx = -1，退出执行

 - 执行 `./x86.py -p loop.s -t 2 -i 100 -a dx=3,dx=3 -R dx` ， 请问`dx`的值是什么？
 > 答：dx初始值是3，循环到dx = -1 < 0，然后去执行thread 1，也是循环到dx = -1停止执行，因此最后dx = -1

 - 执行 `./x86.py -p loop.s -t 2 -i 3 -r -a dx=3,dx=3 -R dx`， 请问`dx`的值是什么？
 > 答：此时采用-r -i产生随机中断，发生中断转去执行另一个thread，直到两个thread的dx都小于0
```
...
> 答：因为bx初始值为3，因此每个线程要循环三次，x最后的值是6
```
...
> 中断间隔为1，因此必然会发生race-condition，最终x = 1
```
2000          Thread 0                Thread 1         
    0   
    0   1000 mov 2000, %ax
