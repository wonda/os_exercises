...
```
clock算法的实现大体上与FIFO相似。
首先，修改page结构，加入clock算法用到的一位访问标志位，初始化为0，被访问时标记为1，这个应该不在页面置换算法的范畴内。
然后，修改函数init_mm，虽然list_entry封装了一个双向链表，但是FIFO中只初始化了一个单向链表，而clock算法应该使用循环链表。同时，加入一个扫描指针，先指向链表的头。
接着，修改函数swap_out_victim，扫描指针不断扫描链表，遇到访问位是1的就标记为0，直到遇到第一个访问位是0的页面，换出该页，并将扫描指针指向它的下一项。
最后，修改函数map_swappable，将新的页面插入到链表中扫描指针指向页面的上一项。
clock算法是LRU的近似，在大部分情况下发生缺页的概率肯定比LRU高，但是考虑到LRU实现复杂，需要硬件支持，所以clock是折衷的选择。
在ucore上实现LRU是可能的，不过因为没有硬件支持，所以很难判断效率是否会比采用其他的置换算法更高。
```
