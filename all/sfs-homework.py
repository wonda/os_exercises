...
class fs:
    ...
    # DONE
        ...
        parentInodeNum = self.nameToInum[self.getParent(tfile)]
        parentBlockNum = self.inodes[parentInodeNum].getAddr()
        self.data[parentBlockNum].delDirEntry(tfile)
        self.inodes[parentInodeNum].decRefCnt()
    ...
    # DONE
        parentInodeNum = self.nameToInum[parent]
        parentBlockNum = self.inodes[parentInodeNum].getAddr()
        if self.data[parentBlockNum].getFreeEntries() == 0:
            return -1
        if self.data[parentBlockNum].dirEntryExists(newfile):
            return -1
        tinum = self.nameToInum[target]
        self.inodes[tinum].incRefCnt()
        self.inodes[parentInodeNum].incRefCnt()
        self.data[parentBlockNum].addDirEntry(newfile, tinum)
    ...
    # DONE
        ...
        if ftype == 'd':
            blockNum = self.dataAlloc()
            self.inodes[inum].setAll(ftype, blockNum, 2)
            self.data[blockNum].setType(ftype)
            self.data[blockNum].addDirEntry('.', inum)
            self.data[blockNum].addDirEntry('..', parentInodeNum)
        self.inodes[parentInodeNum].incRefCnt()
        self.data[parentBlockNum].addDirEntry(newfile, inum)
        return inum
    ...
    # DONE
        if curSize == 0:
            ...
            if blockNum < 0:
                return -1
            self.inodes[inum].setAddr(blockNum)
            self.data[blockNum].setType('f')
            self.data[blockNum].addData(data)
        else:
            blockNum = self.inodes[inum].getAddr()
            self.data[blockNum].addData(data)
